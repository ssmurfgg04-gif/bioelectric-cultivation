...
def main() -> dict:
    ...
    #      evaluated exactly once. THE INSTRUMENT BASE IS exp289's LANDED
    #      BODY (commit c3ca382) REUSED VERBATIM — the register, the
    #      decode, the rebuild, the deposit-reading forms; every exp292
    #      delta is marked `exp292 delta` inline; the ONLY new code is
    #      the NBOR coupling source and the dose ladder {1.0, 2.0, 4.0}
    #      plus the minimal re-plumbing the pre-registered checkpoint
    #      split dictates) ================================================
    import hashlib
    import json
    import subprocess
    ...
    EXPECTED_DOCSTRING_SHA256 = (
        "aebe378304a0edbfd6199ba299bb18f82bc382fcf5e16759cba065ecb204e34d")
    EXPECTED_HEADER_SHA256 = (
        "93e2f298eed1ff59847a8dc7f728136cd19fbbc691a86c3cd912b6f0447702ee")
    ...
    #      pinned floor import restored to -60.0 after; exp292 delta:
    #      the -35.0 reader-line pin is N/A — no stress arm here, the
    ...
    #      exp292 delta: THE DOSE LADDER {1.0, 2.0, 4.0} + THE SOURCES —
    #      pre-named; the g=0.0 control is the ANCHOR, read from
    #      exp289's deposit, never re-run) --------------------------------
    ...
    # exp292 delta — THE DOSE LADDER (the pre-named {1.0, 2.0, 4.0}; the
    # g=1.0 point IS exp289's landed grid point — the anchor) and THE
    # SOURCES (the coupling's read source: "self" == exp289's landed
    # form, "nbors" == the neighbor mean over the projected adjacency
    # support). The g=0.0 control is the anchor, READ from exp289's
    # deposit — no re-run (the pre-registered form).
    DOSE_LADDER = (1.0, 2.0, 4.0)
    HISTORY_SOURCES = ("self", "nbors")
    # THE BRANCH BARS (pre-named at 15dab43, numeric, never fit)
    DOSE_MIN_HOSTS = 10                    # the ">= 10/12 hosts" clauses
    MARGIN = 0.05                          # the exp290 0.05 mV margin form
    ...
    #      zero-reader scan (exp292 delta: the allowed set carries
    #      exp289's landed module TOO — the instrument base this module
    #      reuses; the scan asserts phi_history appears in no file
    #      outside the port + the two instrument modules) ----------------
    ...
    def _zero_reader_scan():
        ...
        in exp289's landed instrument (the base this module reuses), in
        THIS module (the instrument), or in exp293's module (the
        batch-48 sibling pre-registered in the SAME commit 15dab43 —
        the register x stress factorial; its body lands in-flight and
        reads the register under its own pre-registered discipline,
        exactly like this module). No reader anywhere else."""
        hits = {}
        ...
        return {"files_with_phi_history": hits,
                ...
                "allowed_set_disclosure": (
                    "the port + exp289's landed instrument (the base) + "
                    "this instrument + exp293 (the batch-48 sibling "
                    "pre-registered in the same commit 15dab43; its "
                    "in-flight body reads the register under its own "
                    "gates) — no other reader anywhere")}

    zero_reader_scan = _zero_reader_scan()

    # ---- the source deposits (READ-ONLY; sha-recorded BEFORE any
    #      read, re-verified byte-unchanged at the end; exp292 delta:
    #      EIGHT deposits — exp243 + exp256 + exp272 + exp273 + exp282 +
    #      exp287 + exp288 + exp289, the G1 pre-name) ---------------------
    ...
    #      recording additions (a)-(g) + the exp289 activation (h)) with
    #      exactly ONE exp292 addition, disclosed, pure:
    #      (h') THE COUPLING'S READ SOURCE: hist_source — "self" reads
    #          phi_history[i] (exp289's landed form VERBATIM, bit-exact
    #          at the default); "nbors" reads the NEIGHBOR MEAN of
    #          phi_history over the projected adjacency's support (the
    #          exp292 delta — the social form). A pure projection: the
    #          register itself UNTOUCHED, the population path identical
    #          to exp289's, the RNG stream and the state untouched (the
    ...
    def _execute_signed_traced(spec, adjacency, seed, op, budget,
        ...
        for i, src in order:
            ...
            #      (exp289's landed form VERBATIM); "nbors" == the
            #      exp292 delta: the NEIGHBOR MEAN of phi_history over
            #      the projected adjacency's support (the read is a
            #      pure projection — the register itself untouched;
            #      the mean is taken at the write time, the walk
            #      order's causal face) ---------------------------------
            written = theta_new
            if g_ctx > 0.0 and cls[i] == 0:
                if hist_source == "nbors":       # exp292 delta
                    nbr_idx = np.where(np.abs(adjacency[i]) > 0)[0]
                    assert nbr_idx.size > 0, \
                        (f"the NBOR support is empty at cell {i} — the "
                         "neighbor mean is undefined")
                    n_nbors_reads += 1
                    src_val = float(np.mean(c.phi_history[nbr_idx]))
    ...
    # ---- PASS 1 (the SELF ladder — the dose face): the sha-asserted
    #      rebuild + the shadow test suite + 3 doses x 72 fresh decodes
    #      (216) at the production budget with the register live and the
    #      coupling reading phi_history[i] (exp289's landed form); the
    #      G1 ANCHOR asserted fail=STOP per row at g=1.0: the errs + the
    #      trace shas == exp289's deposited g=1.0 grid rows BIT-EXACT ---
    def _compute_pass1():
        _LOCK_LOG.clear()
        rb = _rebuild()
            ...
            for k in hosts:
                ...
                for s in SEEDS_RUN:
                    for inst in DEEP_INSTANCES:
                        ...
                        if g == 1.0:
                            # exp292 delta: THE G1 ANCHOR (fail=STOP per
    ...
    # ---- PASS 2 (the NBOR ladder — the source face): the sha-asserted
    #      rebuild + 3 doses x 72 fresh decodes (216) with the coupling
    #      reading the NEIGHBOR MEAN of phi_history over the projected
    #      adjacency support; the register face + the bookkeeping
    #      asserted fail=STOP per row (the population path identical to
    #      exp289's — the read is a pure projection) ----------------------
    def _compute_pass2():
        _LOCK_LOG.clear()
        rb = _rebuild()
        bases = rb["bases"]
            ...
            for k in hosts:
                ...
                for s in SEEDS_RUN:
                    for inst in DEEP_INSTANCES:
                        ...
                        rec = _decode_grid_row(
                            k, rk, ctx["deep"][inst]["spec"], med, s,
                            ctx["fmax"], g, ctx["A"], hist_source="nbors")
                        assert rec["hist_source"] == "nbors", \
                            "the NBOR arm's source tag drifted"
                        # exp292 delta: the G2 pure-projection face —
                        # the neighbor read happens EXACTLY when the
                        # blend arms (fail=STOP per row)
                        assert rec["n_nbors_reads"] == rec[
                            "n_hist_writes"], \
                            (f"{k} {rk} s{s} g{g}: the NBOR read count "
                             "drifted from the blend count — the read "
                             "is not the pure projection")
                        rows_out.append(rec)
                hs = [r for r in rows_out
                      if r["host"] == k and r["g_ctx"] == g]
                print(f"  [pass2 NBOR g={g} {k}] 6 rows | errs "
                      f"{[r['err'] for r in hs]} | hist writes "
                      f"{hs[0]['n_hist_writes']}/row",
                      flush=True)
        assert len(rows_out) == 216, \
        ...
        assert len(_LOCK_LOG) == 216, \
            f"the S* lock count {len(_LOCK_LOG)} != 216 reads"
        return {"pass": 2, "source": "nbors", "rows": rows_out,
                "ro_before": ro_before, "lock_reads": len(_LOCK_LOG),
                "port_sha256": port_sha_entry,
                "exp142_sha256": exp142_sha_entry}
    ...
    #      verbatim; the merge NEVER re-decodes; exp292 delta: the
    #      control column IS exp289's deposited anchor (read, never
    #      re-run) and the branch faces are the DOSE face and the SOURCE
    #      face, recorded independently) -----------------------------------
    def _assemble(p1, p2, run_form):
        assert p1["pass"] == 1 and p2["pass"] == 2, "payload swap"
        assert p1["source"] == "self" and p2["source"] == "nbors", \
        ...
        rows_self = p1["rows"]
        rows_nbors = p2["rows"]
        assert len(rows_self) == 216 and len(rows_nbors) == 216, \
        ...
        #      walk); the blend count constant across the 6 arm-cells ----
        stream_ok_rows = 0
        reg_ok_rows = 0
        hist_const_counter: dict = {}
        for rows, src_name in ((rows_self, "self"), (rows_nbors, "nbors")):
            for r in rows:
                key = (r["host"], int(r["seed"]), r["row_key"])
        ...
        assert stream_ok_rows == 432 and reg_ok_rows == 432, \
            "the arms' stream/register face drifted"

        # ---- the per ARM-CELL tables (2 sources x 3 doses): the 72-row
        #      mean errs, the per-host paired deltas (BOTH signings
        #      recorded — the anchor-minus-arm form is the house
        #      improvement form, exp290's disclosed convention), the
        #      worst-err face vs the 6.0 bar, the G1 arm-cell counts -----
        cells = {}
        for src_name, rows in (("self", rows_self), ("nbors", rows_nbors)):
            for g in DOSE_LADDER:
        ...
        # ---- THE DOSE FACE (the SELF arm's ladder; the clauses in the
        #      anchor-minus-arm improvement form — the convention that
        #      makes the pre-registered parentheticals true, exp290's
        #      disclosed-convention precedent; BOTH signings recorded
        #      per row above): DOSE-SATURATING iff monotone through
        #      g=2.0 AND impr(4.0) >= impr(2.0) (no reversal) on >= 10/12
        #      hosts; DOSE-REVERSAL iff impr(4.0) < impr(2.0) (a
        #      degradation past the peak) on >= 10/12 hosts --------------
        ...
        dose_clause_rows = []
        for h in hosts:
            mono2 = bool(impr[2.0][h] > impr[1.0][h])
            norev = bool(impr[4.0][h] >= impr[2.0][h])
            rev = bool(impr[4.0][h] < impr[2.0][h])
            n_mono2 += int(mono2)
            n_norev += int(norev)
            n_rev += int(rev)
            n_saturating += int(mono2 and norev)
            dose_clause_rows.append({
                "host": h, "outlier": bool(h in outliers),
                "impr_g1": impr[1.0][h], "impr_g2": impr[2.0][h],
                "impr_g4": impr[4.0][h],
                "monotone_through_g2": mono2,
                "no_reversal_g4_ge_g2": norev,
                "reversal_g4_lt_g2": rev,
                "saturating_clause": bool(mono2 and norev)})
        if n_saturating >= DOSE_MIN_HOSTS:
            dose_face = "DOSE-SATURATING"
        elif n_rev >= DOSE_MIN_HOSTS:
            dose_face = "DOSE-REVERSAL"
        else:
            dose_face = "DOSE-UNRESOLVED"

        # ---- THE BEST SHARED DOSE (the comparison point of the source
        #      face; exp289's tie-break form combined across the arms:
        #      most improving hosts summed, then the more-negative
        #      combined arm-minus-anchor total delta, then the smaller
        #      dose) -------------------------------------------------------
        n_imp_self = {g: cells[f"self|g={g:g}"]["n_improved"]
                      for g in DOSE_LADDER}
        n_imp_nbors = {g: cells[f"nbors|g={g:g}"]["n_improved"]
                       for g in DOSE_LADDER}
        combined_tot = {g: cells[f"self|g={g:g}"]
                        ["total_delta_arm_minus_anchor"]
                        + cells[f"nbors|g={g:g}"]
                        ["total_delta_arm_minus_anchor"]
                        for g in DOSE_LADDER}
        best_d = sorted(DOSE_LADDER,
                        key=lambda g: (-(n_imp_self[g] + n_imp_nbors[g]),
                                       combined_tot[g], g))[0]

        # ---- THE SOURCE FACE (at the best shared dose; the docstring's
        #      literal ARM-minus-ANCHOR err-delta form, exp289's landed
        #      delta convention: SELF-DOMINANT iff the SELF mean delta <
        #      the NBOR mean delta by more than the 0.05 mV margin —
        #      equivalently impr_SELF > impr_NBOR + 0.05 under the
        #      anchor-minus-arm table above; both signings recorded) -----
        md_self = float(cells[f"self|g={best_d:g}"]
                        ["mean_paired_delta_arm_minus_anchor"])
        md_nbors = float(cells[f"nbors|g={best_d:g}"]
                         ["mean_paired_delta_arm_minus_anchor"])
        if md_self < md_nbors - MARGIN:
            source_face = "SELF-DOMINANT"
        elif md_nbors < md_self - MARGIN:
            source_face = "NBOR-DOMINANT"
        else:
            source_face = "SOURCE-SYMMETRIC"
        best_source = "self" if md_self <= md_nbors else "nbors"

        # ---- THE AUDIT FACE: exp288's pooled-R_max class-blindness
        #      re-read at the best arm-cell (never gating) ----------------
        def _pooled_face(grows):
            ...
            return {"per_host": ht, "n_concentrating_hosts": n_conc,
                    ...
                    "bars_source": ("exp288's pre-named audit bars, "
                                    ...
                                    "never gating in exp292")}
        best_cell_rows = [r for r in (rows_self if best_source == "self"
                                      else rows_nbors)
                          if r["g_ctx"] == best_d]
        assert len(best_cell_rows) == 72, "the best arm-cell drifted"
        ...
        #      bookkeeping, re-asserted per arm 72/72) ---------------------
        ...
        g2_counts = {"self": _arm_g2_counts(rows_self),
                     "nbors": _arm_g2_counts(rows_nbors)}
        ...
        g1.update({
            ...
            "reader_pin_disclosure": (
                "N/A — no stress arm in exp292; the floor -60.0 "
                "throughout, asserted at setup and re-asserted at exit "
                "(the exp290 -35.0 reader-line pin does not apply)")})

        g3 = {"branch_dose": dose_face, "branch_source": source_face,
              "n_rows_total": len(rows_self) + len(rows_nbors),
              ...
              "dose_clause_counts": {
                  "n_monotone_through_g2": n_mono2,
                  "n_no_reversal_g4_ge_g2": n_norev,
                  "n_reversal_g4_lt_g2": n_rev,
                  "n_saturating_clause": n_saturating,
                  "DOSE_MIN_HOSTS": DOSE_MIN_HOSTS},
              "n_improved_per_cell": {ck: cells[ck]["n_improved"]
                                      for ck in cells},
              "best_shared_dose": float(best_d),
              "best_shared_dose_rule": (
                  "most improving hosts summed across the arms, then the "
                  "more-negative combined arm-minus-anchor total delta, "
                  "then the smaller dose (exp289's tie-break form, "
                  "combined)"),
              "mean_paired_delta_at_best": {"self": md_self,
                                            "nbors": md_nbors},
        ...
        return {"hosts": hosts, "outliers": outliers, "cluster": cluster,
                "rows_self": rows_self, "rows_nbors": rows_nbors,
                "cells": cells, "dose_clause_rows": dose_clause_rows,
                "control_means": control_means,
                "n_control_provenance": n_control_provenance,
                "dose_face": dose_face, "source_face": source_face,
                "n_saturating": n_saturating, "n_rev": n_rev,
                "n_mono2": n_mono2, "n_norev": n_norev,
                "best_d": float(best_d), "md_self": md_self,
                "md_nbors": md_nbors, "best_source": best_source,
    ...
    #      EXP292_MODE=pass1 (the SELF ladder, 216 decodes) | pass2 (the
    #      NBOR ladder, 216 decodes) | merge — each pass a separate
    #      process, the merge assembling + evaluating the gates once and
    #      NEVER re-decoding) --------------------------------------------
    _MODE = os.environ.get("EXP292_MODE", "")
    _CK = {1: os.path.join(ROOT, "results",
                           ".exp292_pass1.checkpoint.json"),
           2: os.path.join(ROOT, "results",
                           ".exp292_pass2.checkpoint.json")}
    ...
    if _MODE in ("pass1", "pass2"):
        ...
        if tag == 1:
            print("=== exp292 pass1: THE SELF LADDER (the dose face; the "
                  "G1 anchor vs exp289's deposited g=1.0 rows) ===")
            payload = _compute_pass1()
        else:
            print("=== exp292 pass2: THE NBOR LADDER (the source face; "
                  "the neighbor mean of phi_history over the projected "
                  "adjacency support) ===")
    ...
    if _MODE == "merge":
        ...
        print("=== exp292: THE HISTORY DOSE AND SOURCE (where the "
              "carrier breaks, and whose commits protect the boundary) "
              "===")
        print(f"  battery: 2 sources x 3 doses x (12 hosts x 3 seeds "
              f"{list(SEEDS_RUN)} x 2 deep instances at n={N400}) = 432 "
              f"decodes at the production budget {BUDGET}; the g=0.0 "
    ...
    source_face = core["source_face"]
    best_d = core["best_d"]
    md_self = core["md_self"]
    md_nbors = core["md_nbors"]
    best_source = core["best_source"]
    control_means = core["control_means"]
    dose_clause_rows = core["dose_clause_rows"]
    ...
    n_per_cell_ok = bool(len(cells) == 6 and all(
    ...
    g1_pass = bool(
        ...
        and n_per_cell_ok
        and g1["lock_reads_pass1"] == 216
        and g1["lock_reads_pass2"] == 216
        and g1["lock_reads_total"] == 432
        and ro_unchanged)
    g2_pass = bool(
        all(v == 72 for kk, v in g2_counts["self"].items()
            if kk != "n_rows")
        and all(v == 72 for kk, v in g2_counts["nbors"].items()
                if kk != "n_rows")
        and g2_counts["self"]["n_rows"] == 216
        and g2_counts["nbors"]["n_rows"] == 216)
    g3_pass = bool(
        g3["branch_dose"] in ("DOSE-SATURATING", "DOSE-REVERSAL",
                              "DOSE-UNRESOLVED")
        and g3["branch_source"] in ("SELF-DOMINANT", "NBOR-DOMINANT",
                                    "SOURCE-SYMMETRIC")
        and g3["n_rows_total"] == 432
        and g3["n_stream_ok_rows"] == 432
        and g3["n_register_ok_rows"] == 432
        and g3["n_hist_const_rows"] == 72
        and g3["n_control_provenance_ok"] == 72
        and g3["lock_reads_total"] == 432)
    ...
    branch = f"{dose_face} x {source_face}"
    if source_face == "SELF-DOMINANT":
        face_text = (
            f"the SELF-history face carries the boundary decode MORE "
            f"than the neighbor face at the best shared dose "
            f"g={best_d} (the mean paired arm-minus-anchor delta SELF "
            f"{md_self:+.4f} vs NBOR {md_nbors:+.4f} mV — beyond the "
            f"pre-named {MARGIN} mV margin): the cell's own last "
            f"commit is the better carrier")
    elif source_face == "NBOR-DOMINANT":
        face_text = (
            f"the NEIGHBOR-history face carries the boundary decode "
            f"MORE than the self face at the best shared dose "
            f"g={best_d} (the mean paired arm-minus-anchor delta NBOR "
            f"{md_nbors:+.4f} vs SELF {md_self:+.4f} mV — beyond the "
            f"pre-named {MARGIN} mV margin): the social form wins — "
            f"the boundary cells are pulled toward their neighbors' "
            f"commits, not their own")
    else:
        face_text = (
            f"the two source faces are within the pre-named {MARGIN} "
            f"mV margin at the best shared dose g={best_d} (SELF "
            f"{md_self:+.4f} vs NBOR {md_nbors:+.4f} mV): SOURCE-"
            f"SYMMETRIC — the carrier question does not separate on "
            f"this battery")
    if dose_face == "DOSE-SATURATING":
        dose_text = (
            f"DOSE-SATURATING — the SELF deltas improve monotonically "
            f"through g=2.0 and do not reverse at g=4.0 on "
            f"{core['n_saturating']}/12 hosts (>= the pre-named "
            f"{DOSE_MIN_HOSTS}); the carrier does not break below the "
            f"ladder's ceiling")
    elif dose_face == "DOSE-REVERSAL":
        dose_text = (
            f"DOSE-REVERSAL — the improvement degrades past the g=2.0 "
            f"peak on {core['n_rev']}/12 hosts (>= the pre-named "
            f"{DOSE_MIN_HOSTS}): the dose response turns; g=4.0 "
            f"overshoots the useful blend")
    else:
        dose_text = (
            f"DOSE-UNRESOLVED — neither pre-named dose bar met "
            f"(saturating clause {core['n_saturating']}/12, reversal "
            f"clause {core['n_rev']}/12 vs the {DOSE_MIN_HOSTS}/12 "
            f"bar); the counts deposited honestly")

    verdict_body = (
        f"{dose_text}; {face_text}"
        ...
        + "; ".join(
            ...
            for src in ("self", "nbors") for g in DOSE_LADDER)
        + f" | the best shared dose g={best_d} (the combined rule); the "
          f"per-host mean errs at the best arm-cell "
          f"({best_source}) within "
          f"[{min(cells[f'{best_source}|g={best_d:g}']['host_mean_err'].values()):.3f}, "
          f"{max(cells[f'{best_source}|g={best_d:g}']['host_mean_err'].values()):.3f}] "
        ...
        + "; ".join(
            ...
            for src in ("self", "nbors") for g in DOSE_LADDER)
        + f" | THE G1 ANCHOR: the SELF g=1.0 errs reproduce exp289's "
          f"deposited g=1.0 errs BIT-EXACT "
          f"({g1['n_anchor289_err_ok']}/72) with the trace shas "
          f"({g1['n_anchor289_trace_ok']}/72) — the same-walk "
          f"discipline; the arm-cell integrity counts 6 x 4 faces "
          f"72/72; the S* lock reads {g1['lock_reads_total']} "
          f"(2 sources x 3 doses x 72)"
          f" | THE REGISTER FACE (G2): the replay equality "
          f"{g2_counts['self']['n_register_replay_ok']}/72 + "
          f"{g2_counts['nbors']['n_register_replay_ok']}/72, the "
          f"spec-install complement "
          f"{g2_counts['self']['n_register_complement_ok']}/72 + "
          f"{g2_counts['nbors']['n_register_complement_ok']}/72, "
          f"finite 72/72 per arm — the register survives the walk + "
          f"the settle on BOTH arms, the population path identical "
          f"(the NBOR read a pure projection, the neighbor-read count "
          f"== the blend count 216/216)"
          f" | THE AUDIT FACES (never gating): exp288's pooled-R_max "
          f"class-blindness re-read at the best arm-cell — "
          ...
          f"files carry phi_history (the port + the two instrument "
          f"modules only)"
          f" | 12 hosts, a deterministic sha-asserted rebuild "
          f"(graph_path for H0/H1, small_world at the deposited rewire "
          f"seeds), the provenance chains sha-verified 31/31, 8 "
          f"deposits READ-ONLY byte-unchanged, one pass per arm "
          f"({core['run_form']}), floor -60.0")

    # ---- the discipline scans + the deposit form -------------------------
    deposit = {
        "exp": "exp292_history_dose_source",
        "claim": (
            "THE HISTORY DOSE AND SOURCE (batch 48, pre-registration "
            "commit 15dab43; ledger L269's registered nexts (i)+(ii)): "
            "exp289 landed HISTORY-CARRIED — the ctx coupling reading "
            "the SELF-history register improves the decode on 12/12 "
            "hosts at every grid point of exp258's grid, MONOTONE in g, "
            "total -0.3100 mV at g=1.0. Two structural questions, zero "
            "new knobs: (1) THE DOSE — where does the improvement "
            "saturate or break? The ladder extends to the pre-named "
            "{1.0, 2.0, 4.0} (the g>1 blends OVERSHOOT the register "
            "value: written = (1-g)*theta_new + g*history). (2) THE "
            "SOURCE — exp289 read the cell's OWN last commit (the "
            "self-history face); the alternative is the NEIGHBOR-"
            "history face (the cell pulled toward the MEAN last-commit "
            "of its graph neighbors over the projected adjacency "
            "support — the social form: the boundary cells' neighbors "
            "are the unwound tissue whose history is the spec "
            "install). The two forms crossed with the dose ladder in "
            "ONE battery: 2 sources x 3 doses x 72 rows = 432 decodes; "
            "the g=0.0 control is exp289's deposited anchor (read, "
            "never re-run). Branches: the dose face DOSE-SATURATING / "
            "DOSE-REVERSAL (the pre-named 10-of-12 clauses) x the "
            "source face SELF-DOMINANT / NBOR-DOMINANT / "
            "SOURCE-SYMMETRIC (the exp290 0.05 mV margin form) — the "
            "two faces recorded independently"),
        "method": {
            "port": (
                ...
                "entry == exit, recorded); the zero-reader scan asserts "
                "phi_history appears in no file outside the port + "
                "exp289's landed instrument + this instrument"),
            "replica": (
                ...
                "g_ctx == 0.0)"),
            "arms": (
                "SELF — the coupling reads phi_history[i] (exp289's "
                "landed form verbatim; asserted == the deposited "
                "instrument by G1's bit-exact g=1.0 anchor: the errs + "
                "the trace shas 72/72). NBOR — the coupling reads "
                "mean(phi_history[nbrs(i)]) over the projected "
                "adjacency A_ext's support (|A_ext[i]| > 0, the mean at "
                "the write time — the walk order's causal face; the "
                "register itself UNTOUCHED: the read is a pure "
                "projection, the population path identical, the "
                "neighbor-read count == the blend count asserted 216/"
                "216). The arms differ ONLY in the coupling's read "
                "source (the executor's hist_source; default 'self')"),
            "ladder": (
                "the doses {1.0, 2.0, 4.0} on both arms (the g=1.0 "
                "SELF point IS exp289's landed grid point — the "
                "anchor); the g=0.0 control is exp289's deposited "
                "anchor battery (72 errs, walk_steps, n_commits — "
                "read, never re-run; its provenance == exp256's "
                "deposited substituted errs re-asserted 72/72 at "
                "assembly); the walk structure is value-level only "
                "(the stream face: the walked + commit counts == the "
                "anchor's 432/432; the per-row blend count constant "
                "across the 6 arm-cells 72/72)"),
            "branch_rule": (
                "per arm-cell (2 sources x 3 doses) the 72-row mean "
                ...
                "clauses are evaluated on the ANCHOR-MINUS-ARM "
                "improvement form (delta = err_anchor - err_arm; "
                "positive = the arm improves the decode) — the form "
                "that makes the pre-registered parentheticals true: "
                "DOSE-SATURATING iff the SELF deltas improve "
                "monotonically through g=2.0 (impr(2.0) > impr(1.0)) "
                "AND the g=4.0 delta >= the g=2.0 delta (no reversal) "
                "on >= 10/12 hosts; DOSE-REVERSAL iff the SELF delta "
                "at g=4.0 < the g=2.0 delta (a degradation past the "
                "peak) on >= 10/12 hosts. The SOURCE clause is "
                "evaluated in the pre-registered ARM-MINUS-ANCHOR "
                "err-delta form (exp289's landed delta convention): "
                "SELF-DOMINANT iff the SELF mean delta < the NBOR mean "
                "delta at the best shared dose by more than 0.05 mV; "
                "NBOR-DOMINANT iff the reverse by the same margin; "
                "SOURCE-SYMMETRIC otherwise (equivalently impr_SELF > "
                "impr_NBOR + 0.05 under the anchor-minus-arm table). "
                "The best shared dose: most improving hosts summed "
                "across the arms, then the more-negative combined "
                "arm-minus-anchor total delta, then the smaller dose "
                "(exp289's tie-break form, combined). If neither dose "
                "bar is met the face records DOSE-UNRESOLVED with the "
                "counts"),
            "audit_faces": (
                "the worst-err per arm-cell vs the anchor's 6.0 bar "
                "(unchanged); exp288's pooled-R_max class-blindness "
                "re-read at the best arm-cell (exp208's classify "
                "VERBATIM, exp288's pooling, exp288's pre-named bars "
                "CONC_BAR 1.50 / CONC_MIN_HOSTS 10 — audit-only, never "
                "gating), carried next to exp288's deposited face; the "
                "H3/H5 outlier deltas per arm-cell; the shadow test "
                "suite (green, disclosed subprocess)"),
            "run_form": (
                "the default in-process form runs the whole sequence "
                "(may exceed the 570 s cap); the pre-named sandbox "
                "form is the checkpoint split EXP292_MODE=pass1 (the "
                "SELF ladder, 216 decodes) | pass2 (the NBOR ladder, "
            ...
            "scope": (
                ...
                "exp284/exp285/exp286/exp287/exp288/exp289 "
                "precedent); no wall-clock fields")},
        "inputs": {
            name: {
            ...
            for (p, name), role in zip(_dep_names.items(), (
                ...
                "rows (the 72 errs + trace shas the SELF arm must "
        ...
        "rows_nbors": core["rows_nbors"],
        "ladder": {
            "doses": list(DOSE_LADDER),
            ...
            "control_means": control_means,
            "cells": cells,
            "dose_clause_rows": dose_clause_rows,
            "best_shared_dose": best_d,
            "best_shared_dose_rule": g3["best_shared_dose_rule"],
            "n_improved_per_cell": g3["n_improved_per_cell"]},
        "branch": branch,
        "branch_discriminant": {
            "bars": {"DOSE_MIN_HOSTS": DOSE_MIN_HOSTS,
                     "MARGIN": MARGIN},
            "bars_pre_named_at": ("15dab43 — fixed at pre-registration, "
                                  "never fit"),
            "dose_clause_form": (
                "the anchor-minus-arm improvement form (positive = "
                "better) — the form that makes the pre-registered "
                "parentheticals '(no reversal)' and '(a degradation "
                "past the peak)' true; the source clause in the "
                "arm-minus-anchor err-delta form (exp289's landed "
                "convention); both signings recorded"),
            "resolved": {"branch": branch,
                         "dose_face": dose_face,
                         "source_face": source_face,
                         "best_shared_dose": best_d,
                         "best_source": best_source,
                         "n_saturating_clause": core["n_saturating"],
                         "n_reversal_clause": core["n_rev"]},
            "audit_rmax_face_at_best_cell": rmax_best,
        ...
        "determinism": {
            ...
            "decodes": {"self_g1.0": 72, "self_g2.0": 72,
                        "self_g4.0": 72, "nbors_g1.0": 72,
                        "nbors_g2.0": 72, "nbors_g4.0": 72},
            "stream_untouched_proof": (
                "the register is a state variable, not a second RNG "
                "consumer — the population is a pure recording after "
                "the write and the NBOR read a pure projection before "
                ...
                "== the anchor's 432/432; the per-row blend count "
                "constant across the 6 arm-cells 72/72"),
            "two_pass_note": (
                "the exp288 two-pass bit-identity discipline does NOT "
                "carry to this module — G4 pre-names the one-pass-per-"
                "arm form (the exp289 checkpoint-split precedent)"),
    ...
    gates = {
        "G1_the_anchor_and_the_arms_integrity": {
            ...
            "counts": g1,
            "anchor_289": (
                "the SELF g=1.0 errs reproduce exp289's deposited g=1.0 "
                "errs BIT-EXACT 72/72 with the trace shas 72/72 (the "
                "same-walk discipline; asserted fail=STOP per row at "
                "decode); the SELF form's coupling thereby asserted == "
                "the deposited instrument"),
            "arm_cell_integrity": (
                "per arm-cell (2 sources x 3 doses) the finite/A3/"
                "trace-length/commit-count asserts 72/72 (fail=STOP at "
                "decode; the counts recorded per cell)"),
            "lock_reads": (
                f"{g1['lock_reads_total']} (2 sources x 3 doses x 72; "
                f"{g1['lock_reads_pass1']} + {g1['lock_reads_pass2']})"),
            "floor_disclosure": (
                "the -35.0-pin discipline N/A — no stress arm; the "
                "floor -60.0 throughout, asserted at setup, at the "
                "gate assembly and at exit"),
            "source_deposits_read_only_byte_unchanged": ro_unchanged,
            "n_source_deposits": len(READ_DEPS)},
        "G2_the_source_forms_definition": {
            "pass": g2_pass,
            ...
            "zero_knob_definition": (
                ...
                "by G1's bit-exactness); the NBOR form's coupling "
                "reads mean(phi_history[nbrs(i)]) over the projected "
                "adjacency support (the neighbors' last commits; the "
                "support-nonempty assert fail=STOP per armed write; "
                "the neighbor-read count == the blend count 216/216 — "
                "the read is a pure projection, the register itself "
                "UNTOUCHED, the population path identical to exp289's)"),
            "register_replay_per_arm": (
                "the register's replay equality (exp289's G2 form) "
                "re-asserted per arm 72/72: on the write set the "
                "register IS the commit replay BIT-EXACT; off the "
                "write set it IS the spec install; finite 72/72 per "
                "arm; the commit bookkeeping (the exp287 form) 72/72 "
                "per arm")},
        "G3_the_dose_and_source_branch": {
            "pass": g3_pass,
            "bars": (
                "per arm-cell the 72-row mean err and the mean paired "
                "delta vs the g=0.0 anchor; DOSE-SATURATING iff the "
                "SELF deltas improve monotonically through g=2.0 and "
                "the g=4.0 delta >= the g=2.0 delta (no reversal) on "
                ">= 10/12 hosts; DOSE-REVERSAL iff the SELF delta at "
                "g=4.0 < the g=2.0 delta (a degradation past the peak) "
                "on >= 10/12 hosts; SELF-DOMINANT iff the SELF mean "
                "delta < the NBOR mean delta at the best shared dose "
                "by more than 0.05 mV (the exp290 margin form); "
                "NBOR-DOMINANT iff the reverse by the same margin; "
                "SOURCE-SYMMETRIC otherwise. Audit-only, never gating: "
                "the per-host per-arm delta table; the worst-err per "
                "arm vs the 6.0 bar; exp288's pooled-R_max face at "
                "the best arm; the H3/H5 outlier deltas"),
            "resolved": {"branch": branch,
                         "dose_face": dose_face,
                         "source_face": source_face,
                         "dose_clause_counts": g3["dose_clause_counts"],
                         "best_shared_dose": best_d,
                         "mean_paired_delta_at_best": {
                             "self": md_self, "nbors": md_nbors}},
            "stream_face": (
                "the walked counts + the commit counts == the anchor's "
                "432/432 (the pairing's precondition); the per-row "
                "blend count constant across the 6 arm-cells 72/72; "
    ...
    print(f"\n  G1 the anchor + the arms' integrity: "
          ...
          f"(the SELF g=1.0 anchor: the errs == exp289's deposited "
          f"g=1.0 errs bit-exact {g1['n_anchor289_err_ok']}/72 + the "
          f"trace shas {g1['n_anchor289_trace_ok']}/72; the arm-cell "
          f"integrity counts 6 cells x 4 faces 72/72: "
          f"{n_per_cell_ok}; the S* lock reads "
          f"{g1['lock_reads_total']} (216 + 216); 8 deposits READ-ONLY "
          f"byte-unchanged: {ro_unchanged})")
    print(f"  G2 the source forms' definition: "
          f"{'PASS' if g2_pass else 'FAIL'} "
          f"(the SELF form == exp289's landed form (via G1's "
          f"bit-exactness); the NBOR form the neighbor mean over the "
          f"projected adjacency support, the pure-projection face "
          f"216/216; the register replay equality per arm: self "
          f"{g2_counts['self']['n_register_replay_ok']}/72 + nbors "
          f"{g2_counts['nbors']['n_register_replay_ok']}/72; the "
          f"complement per arm "
          f"{g2_counts['self']['n_register_complement_ok']}/72 + "
          f"{g2_counts['nbors']['n_register_complement_ok']}/72; the "
          f"commit bookkeeping 72/72 per arm)")
    print("  G3 the dose + source branch (per-host mean paired deltas "
          "vs the g=0.0 anchor, anchor-minus-arm form; H3/H5 the "
          "outliers):")
    for src in ("self", "nbors"):
        parts = "; ".join(
            f"{d['host']} {d['delta_anchor_minus_arm']:+.3f}"
            f"{'*' if d['improved_strict'] else ''}"
            for d in cells[f"{src}|g=1"]["delta_rows"])
        print(f"      {src} g=1.0: "
              f"{cells[f'{src}|g=1']['n_improved']}/12 improved "
              f"(mean paired "
              f"{cells[f'{src}|g=1']['mean_paired_delta_arm_minus_anchor']:+.4f}"
              f" err-delta) | {parts}")
    print(f"      the dose clauses: monotone-through-g2 "
          f"{core['n_mono2']}/12, no-reversal {core['n_norev']}/12, "
          f"reversal {core['n_rev']}/12, saturating clause "
          f"{core['n_saturating']}/12 (bar {DOSE_MIN_HOSTS})")
    print(f"      the source face at the best shared dose g={best_d}: "
          f"SELF {md_self:+.4f} vs NBOR {md_nbors:+.4f} mV (arm-minus-"
          f"anchor err-delta form; the margin {MARGIN})")
    print(f"      the audit R_max face at the best arm-cell "
          f"({best_source} g={best_d}): "
    ...
    print(f"  G4 discipline: "
          ...
          f"wall-clock fields: {no_wall_clock}; 8 deposits READ-ONLY "
          f"byte-unchanged: {ro_unchanged}; exp142 and the core NOT "
          f"modified; floor -60.0 at exit, the -35.0 pin N/A — no "
          f"stress arm)")
    print(f"\n  BRANCH: {branch} | dose clauses (sat/rev) "
          f"{core['n_saturating']}/{core['n_rev']} of 12 | best shared "
          f"dose g={best_d} | best arm {best_source}")
