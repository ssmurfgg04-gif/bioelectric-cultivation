    ...
    try:
        ...
        def run_instance(j: int) -> dict:
            ...
            for s_idx, (s, dep_err) in enumerate(
                ...
                errs.append(err)
                # S1a: bit-exact at the deposit's rounding (exp142's
                # convention, exp213's replay discipline)
                replay_flags.append(
                ...
                if n_bnd is None:
                    n_bnd = int(bnd.sum())
                # S1b: exp208's decomposition reproduces bit-exactly
                ...
                ref_flags.append(all(
                    ...
                    for pc, rc in zip(dec["per_class"],
                                      ref_dec["per_class"])))
                per_inst.append(dec)
                # THE SHELL ARM (pre-named, zero knobs): the window =
                # the boundary cells PLUS their lattice-neighbor shell
                # on the ring backbone (i +/- 1 mod n for each
                # boundary cell) — the smallest window that restores
                # exp205's geometry; the face reads the SHELL window
                # plane-resolved and the ARMED cells' values are
                # replaced ONLY at the boundary cells (the shell's
                # interior members provide context and are NOT
                # written — the decoded V is kept bit-exactly there).
                n = len(T)
                bnd_idx = np.where(bnd)[0]
                shell = {int(i) for i in bnd_idx}
                for i in bnd_idx:
                    shell.add((int(i) - 1) % n)
                    shell.add((int(i) + 1) % n)
                if n_shell is None:
                ...
                V_face, face_meta = plane_resolved_read(
                    member, shell, [], s, plane_of,
                    sweep=(s == seeds[0]))
                emitted_nodes = sorted(sum(
                    (face_meta["planes"][p]["nodes"]
                     for p in face_meta["planes"]), []))
                assert emitted_nodes == sorted(shell), \
                    "the face's emission is not exactly the shell window"
                bnd_arr = np.array([int(i) for i in bnd_idx], dtype=int)
                V_arm = V.copy()
                V_arm[bnd_arr] = V_face[bnd_arr]   # ONLY the boundary
                # S3: the identity clause — (a) every cell OUTSIDE the
                # shell window bit-unchanged by the arm; (b) the
                # shell's non-boundary members bit-unchanged (context
                # only, never written); (c) together: the boundary
                # cells are the only cells whose values change vs the
                # decoded V (np.array_equal, exp205's clause bound to
                # the shell window's complement)
                shell_mask = np.zeros(n, dtype=bool)
                shell_mask[np.array(sorted(shell), dtype=int)] = True
                outside_shell = ~shell_mask
                shell_only = shell_mask & ~bnd
                ident_flags.append(
                    bool(np.array_equal(V_arm[outside_shell],
                                        V[outside_shell]))
                    and bool(np.array_equal(V_arm[shell_only],
                                            V[shell_only]))
                    and bool(np.array_equal(V_arm[~bnd], V[~bnd])))
                ...
                # (deposited, not gated — the class the excess lives
                # in; comparable to exp213's boundary-only arm)
                e2_arm = (V_arm - T) ** 2
        ...
        if smoke:
            ...
            print(f"  j0 s1: med {rec['median_err']} -> shell-armed "
                  f"{rec['armed_median_err']} "
                  f"(bnd {rec['n_boundary_cells']}, shell "
                  f"{rec['n_shell_cells']}/400)")
            return {"exp": "exp216_context_shell_arm", "smoke": True}
        ...
        for j in js:
            rec = run_instance(j)
            insts.append(rec)
            print(f"  [c6 j{j:2d}] med {rec['median_err']} -> "
                  f"shell-armed {rec['armed_median_err']} "
                  f"(shell {rec['n_shell_cells']}/400, replay "
                  f"{'exact' if rec['replay_bit_exact'] else 'DRIFT'}, "
                  f"identity "
                  f"{'exact' if rec['identity_bit_exact'] else 'BROKEN'})")
        if job in SHARDS:
            shard = {"exp": "exp216_context_shell_arm",
        ...
        # ---- S2: the branch (each clause evaluated exactly once) ------
        ...
        s1_pass = bool(n_replay == n_total
                       and section["n_decomp_exact"] == n_total)
        s2_pass = bool(branch in ("REPAIR", "IMPROVED", "NONE"))
        s3_pass = bool(section["n_identity_exact"] == n_total)
        s4_pass = bool(all_finite)

        gates = {
            "S1": {"pass": s1_pass, "n_replay_exact": n_replay,
                   ...
                   "disclosure": ("errs bit-exact vs exp199's deposit at "
                                  ...
                                  "(float ==, the decode is "
                                  "deterministic) — exp213's B1 "
                                  "verbatim")},
            "S2": {"pass": s2_pass, "branch": branch,
                   ...
                   "boundary_class_rms_contrib_mV": {
                       ...
                       "exp216_replay_unarmed": bnd_before,
                       "shell_armed": bnd_armed}},
            "S3": {"pass": s3_pass,
                   "n_identity_exact": section["n_identity_exact"],
                   "clause": ("every cell OUTSIDE the shell window "
                              "bit-unchanged (np.array_equal); the "
                              "shell's non-boundary members "
                              "bit-unchanged (context only, never "
                              "written); the boundary cells are the "
                              "only cells whose values change vs the "
                              "decoded V; the face's emission asserted "
                              "== the shell window per decode")},
            "S4": {"pass": s4_pass, "rejections": 0,
                   "all_finite": bool(all_finite),
                   "pin_restore_asserted": True}}
        npass = sum(1 for g in gates.values() if g["pass"])
        verdict = (f"{npass}/4 gates (S1 S2 S3 S4) | {branch}")
        print(f"  === {verdict} ===")
        print(f"  shell-armed median {armed_median} vs baseline "
        ...
        deposit = {
            "exp": "exp216_context_shell_arm",
            "claim": (
                "THE CONTEXT-SHELL ARM AT n=400 (L189's registered "
                "next): exp213's refutation localized the face's "
                "failure mode — the plane-resolved read is "
                "CONTEXT-HUNGRY (armed only at the ~130 scattered "
                "canon-boundary cells the sub-window carries almost no "
                "neighborhood and the emission lands 7.7x off plan, "
                "while the same face at exp205's full-window site "
                "HALVED the boundary residual) — the registered "
                "repair: the same face at the boundary cells' CONTEXT "
                "WINDOW, win = the boundary cells PLUS their "
                "lattice-neighbor shell (i +/- 1 for each boundary "
                "cell), the smallest window that restores exp205's "
                "geometry; the pre-named bars UNCHANGED: the tail's "
                "1.30 bar and the interior-bit-exact identity clause "
                "bound to the shell window's complement"),
            "pre_registered": {
                "gates_source": (
                    "module docstring, committed before any run "
                    "(pre-registration d52addd, batch 11; gates S1-S4 "
                    "fixed there, each evaluated exactly once)"),
                "gates": [
                    "GATE-S1 (replay) the c6 battery replays bit-exactly "
                    "vs exp199's deposit (25 x 3) and exp208's "
                    "decomposition reproduces bit-exactly at the "
                    "reference (exp213's B1 verbatim)",
                    "GATE-S2 (the shell arm) the shell-armed errs for "
                    "all 25 instances x 3 seeds; the branch named: "
                    "REPAIR (median <= 1.30 — the tail's bar crossed), "
                    "IMPROVED (median improved >= 20% vs 1.40, bar not "
                    "crossed — the pre-registered nesting disclosed), "
                    "NONE",
                    "GATE-S3 (the identity clause) every cell OUTSIDE "
                    "the shell window bit-unchanged by the arm; the "
                    "face's emission asserted == the shell window; the "
                    "boundary cells are the only cells whose values "
                    "change vs the decoded V",
                    "GATE-S4 (hygiene) zero rejections; all finite; the "
                    "-35.0 pin save/restore asserted (exp199's "
                    "semantics)"],
                "conventions": (
                    "the arm's window = {bnd} u {neighbors(bnd)} on the "
                    "ring backbone (i +/- 1 mod n for each "
                    "CANON-BOUNDARY cell, exp208's classification "
                    "verbatim); the face reads the shell window "
                    "plane-resolved — the canon planes are the "
                    "medium's OWN labeling (labeling_bfs_n on the "
                    "battery's base support); the ARMED cells' values "
                    "are replaced ONLY at the boundary cells — the "
                    "shell's interior members provide context and are "
                    "NOT written (the decoded V kept bit-exactly "
                    "there); crossing = [] (no committed cone exists "
                    "in the battery decode — disclosed); empty plane "
                    "windows are recorded no-ops (disclosed)")},
