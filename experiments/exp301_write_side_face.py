...
def main() -> dict:
    ...
    #      the exp301 pre-registration, byte-for-byte; asserted BEFORE
    #      and AFTER the work) ------------------------------------------
    EXPECTED_DOCSTRING_SHA256 = (
        "0f3000378ef29d679f3ae7f142ebd119337f7f70ee213cdf8ce4b249a732e178")
    EXPECTED_HEADER_SHA256 = (
        "f2e6b262249f74a056ad9270b96495b4310254138496a485d02e510e18bec6c6")
    ...
    assert docstring_sha == EXPECTED_DOCSTRING_SHA256, \
        "docstring drifted from the exp301 pre-registration"
    assert header_sha == EXPECTED_HEADER_SHA256, \
        "header drifted from the exp301 pre-registration"
    ...
    WRITE_BATTERY = (("dormant", 0.0), ("union", 1.0), ("union", 0.5))
    UNION_FACES = ("ctx", "gj", "apop")
    CARRY_CVT_BAR = 0.05                 # mV, the write-side cvt drop
    ...
    N_DECODES = len(WRITE_BATTERY) * 72   # 216
    ...
    def _decode_arm_row(host, row_key, spec, med, seed, fmax, faces,
        ...
        rec = {"host": host, "seed": int(seed),
               ...
               "register": reg, "arm_name": arm_name,
    ...
    def _run_pass(tag, with_suite):
        ...
        rows: list = []
        plan = [(faces, g) for faces, g in WRITE_BATTERY]
        for arm_name, g in plan:
            faces = UNION_FACES if arm_name == "union" else ("ctx",)
            for k in hosts:
                ctx = bases[k]
                ...
                hs = [r for r in rows if r["host"] == k
                      and r["arm_name"] == arm_name]
                print(f"  [{'|'.join(faces)} g={g} {k}] 6 rows | errs "
                      f"{[r['err'] for r in hs]} | cvt "
                      f"{hs[0]['cvt_rms']:.4f} | armed "
    ...
    # ---- THE ASSEMBLY: the write-side faces -----------------------------
    def _assemble(p, run_form):
        ...
        # G1: the dormant anchor vs exp287's deposited commit layer
        drows = [r for r in rows if r["arm_name"] == "dormant"]
        assert len(drows) == 72, "the dormant arm's 72 rows drifted"
        n_digest = n_cvt = n_nc = n_err = 0
        for r in drows:
            k = (r["host"], int(r["seed"]), r["row_key"])
            dep287r = _a287[k]
            dep256r = _a282[k]
            ok_d = bool(r["commit_seq_sha256"]
                        == str(dep287r["commit_seq_sha256"]))
            ok_c = bool(r["cvt_rms"] == float(dep287r["cvt_rms"]))
            ok_n = bool(r["n_commits"] == int(dep287r["n_commits"]))
            ok_e = bool(r["err"] == float(dep256r["err"]))
            assert ok_d, (f"{k}: the dormant re-run's commit digest "
                          "drifted from exp287's deposited")
            assert ok_c, (f"{k}: the dormant re-run's cvt_rms drifted")
            assert ok_n, (f"{k}: the dormant re-run's n_commits drifted")
            assert ok_e, (f"{k}: the dormant re-run's err drifted from "
                          "exp256's deposited")
            n_digest += int(ok_d)
            n_cvt += int(ok_c)
            n_nc += int(ok_n)
        ...
        # G3: THE WRITE-SIDE FACES
        write_faces = {}
        for g in (1.0, 0.5):
            urows = [r for r in rows if r["arm_name"] == "union"
                     and float(r["g"]) == g]
            assert len(urows) == 72, \
                f"the union g={g} arm's rows drifted"
            dmap = {(r["host"], int(r["seed"]),
                     r["row_key"]): r["commit_seq_sha256"]
                    for r in drows}
            n_differ = sum(
                1 for r in urows
                if r["commit_seq_sha256"]
                != dmap[(r["host"], int(r["seed"]), r["row_key"])])
            cvt_u = float(np.mean([r["cvt_rms"] for r in urows]))
            cvt_d = float(np.mean([r["cvt_rms"] for r in drows]))
            drop = cvt_d - cvt_u
            hd = {}
            for h in hosts:
                hu = [r["cvt_rms"] for r in urows if r["host"] == h]
                hdow = [r["cvt_rms"] for r in drows if r["host"] == h]
                hd[h] = float(np.mean(hdow) - np.mean(hu))
            n_imp = sum(1 for h in hosts if hd[h] > 0.0)
            write_faces[f"union_g{g}"] = {
                "n_digests_differ": n_differ,
                "mean_cvt_union": cvt_u,
                "mean_cvt_dormant": cvt_d,
                "cvt_drop": drop,
                "n_hosts_cvt_improved": n_imp,
                "host_cvt_drops": hd}
        # the census face (audit)
        census = {}
        for arm_name, g in WRITE_BATTERY:
            arows = [r for r in rows if r["arm_name"] == arm_name
                     and (float(r["g"]) == g if arm_name == "union"
                          else True)]
            c = {"spec": 0, "canon": 0, "parent": 0}
            for r in arows:
                for t, v in r["src_census"].items():
                    c[t] += int(v)
            census[f"{arm_name}_g{g}"] = c
        branch = "WRITE-SIDE-MARGINAL"
        f10 = write_faces["union_g1.0"]
        if f10["cvt_drop"] > CARRY_CVT_BAR \
                and f10["n_hosts_cvt_improved"] >= CARRY_MIN_HOSTS:
            branch = "WRITE-SIDE-CARRIED"
        elif f10["cvt_drop"] <= 0.0:
            branch = "WRITE-SIDE-ABSENT"

        deposit = {
            "exp": "exp301_write_side_face",
            "claim": ("THE CARRIER'S WRITE-SIDE FACE (batch 55; the "
                      "exp301 pre-registration): does the composed "
                      "carrier change WHAT the program commits? the "
                      "dormant anchor reproduces exp287's deposited "
                      "commit layer bit-exact; the union at the "
                      "composed optimum g=1.0 and at g=0.5: the digest "
                      "face, the cvt face, the census face"),
            "method": {
                "battery": ("the shared 72-row deep-row battery; "
                            "3 arms x 72 = 216 decodes"),
                "arms": [{"arm_name": a, "faces": (list(UNION_FACES)
                           if a == "union" else ["ctx"]), "g": g}
                          for a, g in WRITE_BATTERY],
                "branch_rule": ("WRITE-SIDE-CARRIED iff the union-1.0 "
                                "mean cvt drop > 0.05 mV AND >= 10/12 "
                                "hosts improve; MARGINAL iff in (0, "
                                "0.05]; ABSENT iff <= 0; the digest "
                                "face recorded independently"),
                "audit_faces": ("the census face; the per-host cvt "
                                "drops; the site-write counts")},
            ...
            "rows": rows,
            "dormant_anchor": {
                "n_commit_digest_bit_exact": n_digest,
                "n_cvt_bit_exact": n_cvt,
                "n_commits_bit_exact": n_nc,
                "n_err_bit_exact": n_err},
            "per_arm_g2": per_arm_g2,
            "write_faces": write_faces,
            "census": census,
            "branch": branch,
            "branch_discriminant": {"cvt_bar_mV": CARRY_CVT_BAR,
                                    "cvt_min_hosts": CARRY_MIN_HOSTS},
            "determinism": {
                "form": run_form,
        ...
        deposit["discipline"] = {
            ...
            "pinned_to": "the exp301 pre-registration commit",
            "ro_unchanged": True,
            "floor": PROD_FLOOR,
            "reader_pin_disclosed": READER_PIN_FLOOR,
            "run_form": run_form}

        g1_pass = bool(n_digest == 72 and n_cvt == 72 and n_nc == 72
                       and n_err == 72
                       and p["rebuild_counts"]["n_sha_ok"] == 12
                       and p["rebuild_counts"]["n_288_chain_ok"] == 72
                       and p["lock_reads"] == N_DECODES
        ...
        deposit["gates"] = {
            ...
            "G2_the_forms": {"pass": g2_pass, "per_arm": per_arm_g2},
            "G3_the_write_side_branch": {
                "pass": g3_pass, "branch": branch,
                "faces": write_faces,
                "bars": {"cvt_drop_mV": CARRY_CVT_BAR,
                         "cvt_min_hosts": CARRY_MIN_HOSTS}},
        ...
        deposit["verdict"] = (
            ...
            f"{4 - n_pass} REFUTE) | BRANCH: {branch} | the union-1.0 "
            f"cvt drop {f10['cvt_drop']:+.4f} mV "
            f"({f10['n_hosts_cvt_improved']}/12 hosts) | the digests "
            f"differ {f10['n_digests_differ']}/72")

        print(f"\n=== exp301: THE CARRIER'S WRITE-SIDE FACE ===")
        print(f"  G1 the dormant anchor: "
              f"{'PASS' if g1_pass else 'FAIL'} (exp287's commit layer "
              f"reproduced bit-exact: the digests {n_digest}/72, the "
              f"cvt {n_cvt}/72, the n_commits {n_nc}/72, the errs "
              f"{n_err}/72; the S* lock {p['lock_reads']}/{N_DECODES}; "
        ...
        print(f"  G2 the forms: {'PASS' if g2_pass else 'FAIL'} "
              ...
              f"writes seed-invariant)")
        print("  G3 the write-side faces:")
        for key, f in write_faces.items():
            print(f"      {key}: the digests differ "
                  f"{f['n_digests_differ']}/72 | the cvt drop "
                  f"{f['cvt_drop']:+.4f} mV "
                  f"({f['n_hosts_cvt_improved']}/12 hosts)")
        print(f"      the census (audit): {census}")
        print(f"      -> BRANCH: {branch}")
        print(f"  G4 discipline: {'PASS' if g4_pass else 'FAIL'} "
              f"(deterministic, form {run_form}; no wall-clock fields; "
              f"16 deposits READ-ONLY byte-unchanged; exp142 + the "
              f"core NOT modified; floor -60.0 at exit)")
        print(f"\n  BRANCH: {branch}")
    ...
    for arm_name, g in WRITE_BATTERY:
        ...
        for k in hosts:
            ...
            hs = [r for r in rows if r["host"] == k
                  and r["arm_name"] == arm_name]
            print(f"  [{arm_name} {k}] 6 rows | errs "
                  f"{[r['err'] for r in hs]} | cvt "
                  f"{hs[0]['cvt_rms']:.4f} | armed "
                  f"{hs[0]['n_arm_writes']}/row | reg replay "
                  f"{sum(r['register']['replay_ok'] for r in hs)}/6",
    ...
    p = {"rows": rows,
         ...
         "test_suite": suite,
         "zero_reader_scan": zero_reader_scan}
    deposit = _assemble(p, run_form="in-process all (3 arms x 72 in "
                                    "one process)")
