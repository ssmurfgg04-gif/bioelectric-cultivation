#!/usr/bin/env python3
"""exp134 — THE THETA-EXPOSURE DEADLINE (ledger L113.s registration: the
...
PRE-REGISTERED GATES:
  ...
  TE-A1  (the default anchor) the never-silenced limit (t* = T_pre) at
         gamma 1 on the torus predicts > 6.0 (the default run breaks,
         exp122's 10.00 refusal).
  TE-G1  (the torus fit) ONE kappa satisfies ALL: gamma-4 pred(48) > 6
  ...
  TE-G2  (zero-free transfer) at the interval's mid-kappa, every REAL
  ...
  TE-G3  (the two failure lessons, structural) (a) SATURATION: the
         drift is bounded — report max|delta-theta| on dI at T_pre vs the
...
Two-sided statements: TE-G1 refuted => the clamp-anchored linearization
is also not the law (the temporal arc's analytic family narrows to
explicitly hybrid models); TE-G2 refuted with TE-G1 passing => the
boundary-drift map is substrate-dependent (the per-substrate transfer
coefficient becomes the next registered object).

PRE-REGISTRATION (verbatim from ledger L113): the model form is
exp132's with the exposure block swapped (theta for V on the boundary
intact cells), decided BEFORE running. Gates TE-A1/TE-G1/TE-G2/TE-G3
are exp132's DL-A1/G1/G2/G3 under the swapped exposure.
...
OUT = os.path.join(ROOT, "results", "exp134_theta_exposure_deadline.json")
...
def boundary_damage(adj, gamma, t_star, t_grid):
    """D(gamma, t*) = sum over dI of int_0^T_pre |delta-theta_i(t)| dt
    (the THETA-exposure: the walk's inheritance reads c.theta[src],
    not V — the exp132 refutation's registered correction)."""
    n = adj.shape[0]
    ...
    for k, tg in enumerate(t_grid):
        ...
        traj.append(np.abs(x[len(I):2 * len(I)][bnd_cols]))
    if traj:
...
def main() -> dict:
    ...
    # ---- TE-G1: the torus fit (kappa interval)
    ...
    dl_g1 = kmin < kmax
    print(f"  TE-G1 torus fit: kappa in ({kmin:.4f}, {kmax:.4f}) -> "
    ...
    if dl_g1:
        ...
        print(f"  TE-G2 transfer: {agree}/{tot} cells agree "
              f"({frac:.3f}) mismatches {mism[:8]} -> "
              f"{'PASS' if frac == 1.0 else 'REFUTED'}", flush=True)
    else:
        gates["DL_G2_transfer"] = None
        print("  TE-G2: VOID (no feasible kappa)", flush=True)

    # ---- TE-G3: the structural lessons
    ...
    gates["DL_G3_structure"] = {
        "saturation_max_dTHETA_boundary_mu_active": round(sat_mu, 3),
        "saturation_max_dTHETA_boundary_silenced": round(sat_sil, 3),
    ...
    print(f"  TE-G3: saturation {sat_mu:.2f} mV (mu-active fixed point, "
    ...
    result = {
        "exp": "exp134_theta_exposure_deadline",
