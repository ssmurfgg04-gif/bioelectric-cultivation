...
def main() -> dict:
    ...
    EXPECTED_DOCSTRING_SHA256 = (
        "f0cd08b7699d216ab7dafc71aa9af145a2c6c998b7424bb9f3eab65596af4ec3")
    EXPECTED_HEADER_SHA256 = (
        "80d05f9ed56065878b30b30a13ede4f8271ceef22d7ccfc01d5b189697ec61bf")
    ...
    assert docstring_sha == EXPECTED_DOCSTRING_SHA256, \
        "docstring drifted from the exp299 pre-registration"
    assert header_sha == EXPECTED_HEADER_SHA256, \
        "header drifted from the exp299 pre-registration"
    ...
    G_SUB = 0.5                           # the sub-saturation dose
    ...
    def _exp299_anchor():
        # the deposited g=0.5 rows per face: ctx <- exp289's grid rows
        # (the ctx form's own dose ladder landed there); gj/apop <-
        # exp297's ladder rows (the dose face batch)
        anch = {}
        ctx_rows = [r for r in dep289["grid_rows"]
                    if float(r["g_ctx"]) == 0.5]
        assert len(ctx_rows) == 72, \
            "exp289's deposited g=0.5 grid rows drifted"
        m = {}
        for r in ctx_rows:
            k = (r["host"], int(r["seed"]), r["row_key"])
            assert k not in m, f"duplicate exp289 g=0.5 row {k}"
            m[k] = {"err": float(r["err"]),
                    "trace_sha256": str(r["trace_sha256"])}
        anch["ctx"] = m
        for key0 in ("gj", "apop"):
            arows = [r for r in dep297["rows"]
                     if r["arm"] == key0 and float(r["g"]) == 0.5]
            assert len(arows) == 72, \
                f"exp297's deposited {key0} g=0.5 rows drifted"
            m = {}
            for r in arows:
                k = (r["host"], int(r["seed"]), r["row_key"])
    ...
    def _assemble(p, run_form):
        ...
        # G1: the single-face anchors vs the deposited g=0.5 rows
        anch, base_ctrl = _exp299_anchor()
        anchor_tallies = {}
        ...
        deposit = {
            "exp": "exp299_subsaturation_faces",
            "claim": ("THE CARRIER AT THE SUB-SATURATION FACES "
                      "(batch 53; the exp299 pre-registration): the "
                      "exp298 composition at g=0.5 where the two forms "
                      "genuinely differ -- the blend re-asserts the "
                      "register value, the sigma form only shrinks the "
                      "noise; does the blend add value where the sigma "
                      "leaves noise, or does the re-assertion subsume "
                      "at every dose?"),
            "method": {
                "battery": ("the shared 72-row deep-row battery at "
                            "g=0.5; 4 arms x 72 = 288 decodes"),
        ...
        deposit["discipline"] = {
            ...
            "pinned_to": "the exp299 pre-registration commit",
            "ro_unchanged": True,
            "floor": PROD_FLOOR,
        ...
        print(f"\n=== exp299: THE CARRIER AT THE SUB-SATURATION "
              f"FACES ===")
        print(f"  G1 the anchor ladder: "
              f"{'PASS' if g1_pass else 'FAIL'} (the deposited g=0.5 "
              f"single-face rows reproduced bit-exact: "
