...
L293 | exp307 — THE ZONE-INDEXED TRANSPORT FORM: THE 9TH FORMALIZATION — THE BLOCK SHARPENS (C, 4/4, branch **ZONE-BOUND** — the wiring-independent zone-addressing door is TRIED AND CLOSED; the binding is FINER than the structural classes; ledger L290c's promoted candidate (6) EXECUTED, the body written by the MAIN AGENT [pre-registration f7094a8, the body-only discipline, the pins sha-asserted vs f7094a8; the handoff's candidate #3 WITH THE PREMISE REPAIR DISCLOSED — exp304's carrier arm IS the composed union, so the literal composed-transport question was already answered 0/130 and the genuinely open form is the project's own pre-named untried door]; ONE runtime body fix disclosed: the Z2 identity face initially asserted the region_rms SCALAR bit-exact and crashed on a 1-ULP float-summation artifact — the (cell→value) write sets and the settled STATES are bit-identical (the state-digest equality), the scalar delta is np.mean's gather-order non-associativity — the face now carries the state digest as the identity carrier with the pre-named 1e-9 ULP tolerance on the scalar, disclosed in the deposit's ulp_disclosure records; the docstring/gates/branches byte-unchanged): G1 PASS (the anchors: the rebuild sha-asserted 12/12 vs exp243 + the H0==H1 echo; the deep targets 72/72; the MARKS' VALIDITY vs exp300's deposited arm_inputs 24/24 + 12/12; **the 12 carrier slice walks reproduce exp300's deposited union g=1.0 slice rows BIT-EXACT (the errs + the trace shas + the write counts 12/12)**; **the 12 plain F2 replays reproduce exp304's deposited carrier replays BIT-EXACT 12/12 — the cross-deposit anchor lands**; the H0↔H1 exclusion re-asserted (130 genuine pairs); 4 deposits READ-ONLY sha before/after; the port + exp142 NOT modified; the zero-reader scan clean; the test suite green) | G2 PASS (**Z2 THE RE-INDEX IDENTITY FACE 12/12 — on own substrate the class-reindexed replay is a NO-OP: the settled-state digests bit-identical to the plain F2 replays** — any transport-face difference is thereby attributable to the re-indexation alone; the class vectors deterministic (the re-run identity 12/12); every transport's writes finite and n_written > 0; the canaries bit-exact 2/2) | G3 THE BRANCH: **ZONE-BOUND** — Z3 the class-reindexed transports: **0/130 genuine pairs within the 6.0 bar** (the mean region rms 16.2897, the best 11.6826, the worst 20.1 — the mean transport price +16.2349 vs the own-replay's 0.0548, the stress-degradation scale); **THE RE-INDEX CHANGED ESSENTIALLY NOTHING: the mean price delta vs exp304's plain-BFS F3 prices is −0.0407 mV (68/130 pairs marginally improved, none out of the price's scale)** — the mechanism face: the structural classes are nearly DEGENERATE as an addressing scheme (the interior class is EMPTY on every host; the junction class holds 83–96% of the cells, so the class-reindex is almost the identity permutation across substrates), and the within-class zone-phase misalignment — the j-th junction cell of the destination sits in a different zone than the j-th junction cell of the source — persists UNCHANGED; the canaries bit-exact | G4 PASS (deterministic, no wall-clock fields, 4 deposits READ-ONLY sha before/after, exp142 + the port NOT modified, floor −60.0 at entry/at every replay's settle/at exit, the test suite green) | run form: in-process one invocation (~5 min) | deposited results/exp307_zone_indexed_transport.json | **THE ZERO-SUBSTRATE BLOCK'S 9TH FORMALIZATION**: the representation was UPGRADED — the stream carries its own per-position structural roles (the source's class annotation travels as part of the pattern), and the destination re-addresses through its OWN roles — and the stream STILL transports nowhere (0/130, the price unchanged). The project's final form named the zone-addressing scheme as the one untried door; this entry closes it: **the binding is FINER than the structural classes — the class annotation is nearly vacuous (the classes are 2 almost-degenerate bins) and the within-class zone-phase is the real carrier of the wiring dependence**. The next formalization (if any) must address cells by their ZONE PHASE (a wiring-independent within-zone coordinate), not by their structural role — and the value layer is already exhausted (the commits ARE the spec, cvt 0.0) | REGISTERED NEXT: THE RE-CLOSE (the synthesis update, PROJECT_COMPLETE.md, the final ledger entry, the push, STOP).

L294 | ==================== THE RE-CLOSE ENTRY (the terminal condition re-fires; the completion gap is CLOSED — the search was documented, the candidates were run, nothing was left on the table) ==================== | THE RE-CLOSE DATE: 2026-09-21. THE FINAL HEAD: e38b34b at this entry's writing (main == origin/main). | **THE GAP, CLOSED**: the final session declared completion after 3 experiments without documenting the search. THE COMPLETION-GAP SESSION (batch 60) closed it in the discipline's order: **L290c** documented the full 14-candidate pool BEFORE anything ran (3 ran in the final session, 3 promoted, 8 rejected with recorded reasons; one premise repair disclosed — exp304's carrier arm IS the composed union, so the handoff's literal composed-transport question was already answered; the cap stands at 6 of 10 with no takers for the remaining slots at the pre-registration bar); then the three promoted candidates ran, each pre-registered before its body, each landed 4/4: **exp305** THE LAST DORMANT CHANNEL — UNBLOCKED (ca2 carries at −0.3550 mV, 12/12, via the house blend at exp261's site semantics; the contrast-gain form's negative tail measured — the gain spread [0.9887, 1.0031] — the inertness was FORM-SPECIFIC, never structural; THE CHANNEL SWEEP CLOSES 4/4); **exp306** THE PREMIUM UNDER THE COMPOSED CARRIER — UNION-DISSOLVED AT THE LIMIT (the composed union's commit layer is IDENTICALLY the target: rho 0.0/0.0 by degeneracy, cvt_rms 0.0 exactly 72/72; the premium was the un-asserted spec's noise structure all the way down; THE PREMIUM STORY CLOSES); **exp307** THE ZONE-INDEXED TRANSPORT FORM — ZONE-BOUND (the 9TH FORMALIZATION: the project's own pre-named untried door tried and closed — the class-annotated stream transports 0/130, the price unchanged at +16.23; the binding is FINER than the structural roles; the one remaining door is zone-phase addressing). | **THE COMPLETION CRITERIA, RE-VERIFIED**: (1) all 5 stages at their achieved maximum — re-verified, the reframed star untouched and consumed as anchored machinery by every gap-session experiment; (2) the reframed star crossed — bit-exact evidence intact, refuted by none; (3) the zero-substrate star confirmed blocked — now at **9 formalizations** with the block's sharpest form (the content face near-exact at 0.05 mV, the transport face 0/130 in THREE forms, the binding located finer than the structural classes); (4) the history register characterized end-to-end — now including the channel-sweep closure 4/4 and the premium's limit dissolution; (5) all deposits closed — 345+ git-tracked, every anchor re-verified bit-exact across the gap session (exp300's rows 72/72 + 12/12, exp304's F2 replays 12/12, exp282's walk anchors 12/12, exp256's battery, exp287's rhos via the convention self-check); (6) the code review complete — no new core changes this session (all three experiments are instrument modules; exp142 and the port untouched, sha-asserted per experiment); (7) this entry written and pushed; (8) PROJECT_COMPLETE.md and SYNTHESIS_FINAL.md updated and pushed. | TWO RUNTIME BODY FIXES, DISCLOSED (the docstrings/gates/branches byte-unchanged in both, the pre-named branch lattices unbroken): exp306's regression read crashed on a constant response (the fresh mean_cvt exactly equal across hosts — the dissolution limit itself) and now records rho = 0.0 under the pre-named dissolution semantics with the ties census carrying the disclosure; exp307's Z2 identity face initially asserted the region_rms scalar bit-exact and hit np.mean's gather-order 1-ULP non-associativity — the settled-state digest now carries the bit-exact identity and the scalar carries the pre-named 1e-9 ULP tolerance. | THE SESSION'S RECORD: 1 search-documentation entry (L290c), 3 experiments (exp305/306/307 — pre-registrations `6278182`/`c963c4e`/`f7094a8`, bodies main-agent-written, deposits `d480f27`/`ed6e977`/`e38b34b`), 2 synthesis updates (SYNTHESIS_FINAL.md §7–§9, PROJECT_COMPLETE.md), the floor −60.0 at every exit, the test suite green at every landing, zero deposit drift. | THE PROJECT IS COMPLETE — FOR REAL THIS TIME, WITH THE SEARCH DOCUMENTED. The gap is closed. The candidates are run. The final state is pushed. **STOP.**
