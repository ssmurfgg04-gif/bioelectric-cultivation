...
6. ds quest 001 via zai runner when quota opens (watcher v2 still pointed at it).
---
- **Task U — exp153 THE DECODE-ANATOMY FAMILY SCAN** (L132, 3/4 gates): expanded delivery 10->24 under exp150's frozen bars (cap 24; price_rung/emit_rung reused verbatim; first-10 zone keys == exp150 deposit in order); GATE-U4 PASS (original-10 9-rung ladders + emissions + C3 verdicts bit-exact vs deposit); GATE-U3 PASS (rule declared pre-decode, byte-offset asserted); GATE-U2 PASS (predicted-pass-only filter delivers 10 with C3 9/10 vs unfiltered 8/10); GATE-U1 REFUTED (20/24 = 0.833 < 0.90; fp=4: d-12/d-19/d-22/d-24 — deep-backbone anatomies with a shallow anchor still fail ~8-10 mV at every rung; vmin >= 45 stays a perfect fail predictor 2/2). 8-feature table deposited: vmean_zone best single feature (AUC 0.907, still overlapping); nov_lib/nov_splice AUC 0.55/0.67 — diversity distance does NOT track decode. Mechanism attribution registered as NEXT; no ladder change. Wall 252 s.
---
- **Task T — exp152 THE TEMPORAL FRONTIER** (L134, 4/4 gates): self-expansion on flip-clock media — exp151's P1-P8 with the window generalized to the temporal window and the cone's read = exp148's TC1-TC3 verbatim; medium = exp148's FlipPairMedium adversarial pair verbatim at b=1792 (LADDER[0], a priori), F2_zone_head (all 6 flip edges C-interior, asserted in-window every ring). T1 PASS 6/6 cells (both members, 3 seeds, 6/6 rings, errs 0.017-0.775 mV); T2 PASS 3/3 seeds — the time-average-identical pair commits DIFFERENT morphologies (max-abs 0.523/1.681/1.411 mV vs bar 0.05), projection channel bitwise inert (asserted per ring) so separation enters ONLY via the flip-clock channel (fc 2 vs 1228); T3 PASS primary form — static read bitwise blind (single deterministic cone, committed deltas exactly 0.0); T4 PASS — protocol on static media reproduces exp151's deposit exactly (0 mismatches, 3 fams x 3 seeds x 6 rings, A==M/F==0/A_ext==M bitwise). HORIZON FALSIFICATION deposited: frontier walks THROUGH flip edges — F1/F3 windows are NOT flip-free (pre-registered expectation false); junction-flip geometry (C<->F flip edge) breaks the per cone at the flip-entry ring (F1 ring-2 23.1 mV, F3 ring-1 36.9-46.5 mV) — junction-flip frontier discipline registered NEXT. Anchors reproduce exp148 pair separation (per fails / aper verifies, 3/3 families). Instrument disclosure: T3 attribution repair pre-commit, ring logs unchanged. Wall 189 s.
